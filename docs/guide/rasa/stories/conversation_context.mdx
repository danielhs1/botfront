...
Slots comes in different types. **any** slots can be used only to store information,
...
> **Note**:
>
> An any slot **balance** should also be created (not shown in the video)
...
any are slots that are only used to store information. They cannot be used to branch conversations.
